...
S~ohledem na situaci nebylo možné testovat s~většinou osob osobně (s~nahlížením na jejich interakci s~aplikací), a je ted možné se spoléhat jen na reakce, které testující poskytli. I~kvůli tomu nebylo možné získat přepodkládaný vzorek z~řad studentů, většina testujících tedy byli lidé z~věkové skupiny 23-30 let, část z~nich učitelé či studenti pedagogických směrů.

Celkové reakce na prototyp byly kladné -- testující se v~aplikaci bez dalších pokynů snadno zorientovali a během několika minut zvládli misi vyřešit. To ukazuje na jednoduchost úvodní mise, to ovšem hodnotili spíše neutrálně nebo pozitivně -- náročnost jim připadala přiměřená.
